...
export class JobHandler implements IJobHandler {
  ...
  register(bot: Bot) {
    ...
    const paginationRegex = new RegExp(`^${PAGINATION_KEYBOARD_PREFIX}:(\\d+)$`)
    bot.callbackQuery(paginationRegex, context => this.paginationHandler(context))
  }
  ...
  private async handle(context: Context, page: number = 1): Promise<void> {
      ...
      const keyboard = this.keyboard.create(page, totalPages, PAGINATION_KEYBOARD_PREFIX)
